...
public class BarcodeScannerViewController: CodeScannerViewController {
    ...
    public func startBarcodeScanning() {
        super.startScanning(metadataObjectTypes: [.ean13, .code128])
    }

    // Transition to a new view controller to display the barcode and bounding box
    ...
    public override func metadataOutput(_ output: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        guard let metadataObject = metadataObjects.first as? AVMetadataMachineReadableCodeObject,
              let codeString = metadataObject.stringValue, !super.codeDetected else { return }

        super.codeDetected = true
        captureSession?.stopRunning()
